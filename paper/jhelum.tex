...
\begin{abstract}\noindent % trust me
Simulations of tidally disrupting Milky Way satellites produce long and thin---nearly one-dimensional---stellar streams. 
Using astrometric data from the Gaia second data release and photometry from the Dark Energy Survey, we demonstrate that Jhelum, a stellar stream in the inner halo, is a two-dimensional structure.
Spatial distribution of highly probable Jhelum members reveals a dense thin component and an associated diffuse, vertically offset component.
The two Jhelum components have indistinguishable proper motions and a similar ratio of blue straggler to blue horizontal branch stars.
These indicate a common origin for Jhelum components, but while the best-fit orbit to the narrow component also traces the Indus stream, it does not capture Jhelum's wide component.
Substructure in the Jhelum progenitor or precession of its tidal debris in the Milky Way potential can explain the observed structure of Jhelum, and future spectroscopic data can distinguish between these formation scenarios.
...
Interestingly, the $\sim0.5\,\rm{Gyr}$ old orbital segment traces the Indus stream (thick red line, based on the sky positions and typical distance reported in \citealt{shipp2018}).
Originally suggested by \citet{shipp2018} based on their physical proximity, Indus being another wrap of the Jhelum stream would also be consistent with the massive progenitor inferred from the high BS to BHB ratio.
...
These discoveries signal the dawn of a new era, in which we resolve the internal structure of thin stellar streams and use it to trace the fine structure of their formative environment.
