...
Blue horizontal branch (BHB) and blue straggler (BS) stars are present in Jhelum, with a low contamination rate from field stars (bottom panels of Figure~\ref{fig:properties}).
...
Going forward, detailed chemical abundances from spectroscopy will be crucial for definitively confirming the single-progenitor scenario.
...
- running median 
- the stream track deviates slightly from the great circle not much gradient either along the stream
- gray line - whole stream median, gray mad
- two components: red and blue, shown in figure~\ref{fig:components}, median and mad for errorbars
- despite being offset by x deg spatially, the proper motions are remarkably similar
- dispersion also similar, but uncertainties large
...
while the insets present likely Jhelum members in these coordinates.
}
