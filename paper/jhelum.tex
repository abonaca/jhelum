...
The bottom panel zooms in on the stream's current location, and also shows the distribution of likely Jhelum members (assuming a constant distance of 13\,kpc).
On average, the orbit matches the narrow Jhelum component, but they are somewhat misaligned because the best-fit orbit has a small gradient in distance along the stream, which is currently unresolved in the data.
Jhelum's wide component is $\sim0.5\,\rm kpc$ further from the Galactic plane than its narrow component.
None of the previous orbital wraps pass through the wide component, arguing against the two components being debris of a single progenitor released at subsequent pericentric passages.
...
%  if close to the disk on approx circular orbit (Dehnen2018), or if pericenter is close to the center of the Galaxy (eccentric orbit at some angle wrt the disk; Erkal2016) -- eccentricity = 0.5, zmax = 30 kpc; -- not captured in the streakline model
 }
...
%  More quantitatively, the Lyapunov time of Jhelum's orbit, which sets the time when chaos becomes dynamically important, is $\gg t_{Hubble}$.
...
- indus on the same orbit highly suggestive -> orbit probably ballpark ok
- would argue against major perturbations from chaos or time-dependence, or the debris being fully phase mixed
...
- still on the table for 2 jhelum components: gc and dwarf, progenitor substructure, plane precession
- spectroscopy will help -- test single progenitor through chemical abundances
- can distinguish between different single-progenitor scenarios based on mean radial velocity trends and also velocity dispersion
